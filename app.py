...
def apply_login_background():
    ...
    - Carga assets/fondo_login.png y la convierte a base64.
    ...
    - El resto del CSS se lee desde assets/login_style.css.
    """
    ...
    try:
        _login_css = open("assets/login_style.css", encoding="utf-8").read()
    except OSError:
        _login_css = ""  # fallback silencioso si el archivo no existe
    ...
    """.replace("{_login_css}", _login_css), unsafe_allow_html=True)
...
# ─── CSS Tecnológico (panel interno post-login) ─────────────────────────────
try:
    TECH_CSS = "<style>" + open("assets/style.css", encoding="utf-8").read() + "</style>"
except OSError:
    TECH_CSS = ""  # fallback silencioso si el archivo no existe
...
def _migrate_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS biological_indicators (
        ...
        bi_lot            TEXT,
        bi_brand          TEXT,
        incubation_start  TEXT NOT NULL,
        incubation_hours  INTEGER NOT NULL DEFAULT 24,
...
def dashboard():
    ...
    bi_pend=query_df("SELECT COUNT(*) as n FROM biological_indicators WHERE result='Pendiente'")
    ...
    if bi_pend_n:
        ...
        n_over = int(bi_over.iloc[0]["n"]) if not bi_over.empty else 0
        if n_over:
            st.error(f"⏰ {n_over} IB(s) con tiempo de incubación **VENCIDO**. Registre el resultado.")
        else:
            st.warning(f"🧫 {bi_pend_n} IB(s) en incubación. Recuerde registrar el resultado al vencer el plazo.")
